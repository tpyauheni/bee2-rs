...
mod tests {
    ...
    fn test_sign_verify_b2() {
        ...
        bign_key.validate_keypair().unwrap();
        bign_key.validate_public_key().unwrap();
        let hash1 = [201, 65, 149, 231, 9, 62, 52, 84, 31, 111, 216, 198, 52, 242, 250, 19, 186,
            216, 130, 8, 100, 118, 188, 122, 34, 162, 32, 175, 42, 221, 57, 30, 152, 157, 99, 6,
            241, 148, 66, 200, 37, 73, 31, 178, 177, 139, 74, 44];
        let signature1 = bign_key.sign(&hash1, &mut rng).unwrap();
        let signature2 = bign_key.sign(&hash1, &mut rng).unwrap();
        // TODO: Check if it's fine that signature is the same despite function requiring `rng`.
        // assert_ne!(signature1, signature2);
        ...
        bign_key.verify(&bign_key.public_key, &hash1, &signature2).unwrap();
        // TODO: Check if that signature really is valid
        let valid_signature = [250, 72, 23, 69, 80, 129, 196, 35, 120, 252, 222, 204, 38, 156,
            247, 26, 173, 35, 131, 5, 231, 141, 247, 183, 184, 62, 220, 241, 156, 194, 153, 44,
            105, 145, 4, 124, 72, 74, 174, 48, 72, 76, 17, 134, 111, 105, 57, 10, 55, 61, 29,
            191, 24, 93, 189, 87, 233, 151, 244, 184, 220, 214, 254, 194, 123, 129, 188, 76, 156,
            212, 199, 148];
        bign_key.verify(&bign_key.public_key, &hash1, &valid_signature).unwrap();
        let invalid_signature = [250, 72, 23, 69, 80, 129, 196, 35, 120, 252, 222, 204, 38, 156,
            247, 26, 173, 35, 131, 5, 231, 141, 247, 183, 184, 62, 220, 241, 156, 194, 153, 44,
            105, 145, 4, 124, 72, 74, 174, 48, 72, 76, 17, 134, 111, 105, 57, 10, 55, 61, 29,
            191, 24, 93, 189, 87, 233, 151, 244, 184, 220, 214, 254, 194, 123, 129, 188, 76, 156,
            212, 199, 149];
    ...
    fn test_sign_verify_b3() {
        ...
        bign_key.validate_keypair().unwrap();
        bign_key.validate_public_key().unwrap();
        let hash1 = [205, 15, 121, 40, 31, 33, 13, 6, 163, 218, 44, 52, 168, 249, 227, 209, 13,
            220, 185, 115, 173, 151, 35, 216, 109, 108, 219, 2, 251, 81, 148, 197, 77, 94, 21,
            207, 100, 238, 254, 191, 34, 11, 172, 133, 101, 65, 42, 148, 230, 164, 111, 108, 240,
            100, 40, 132, 128, 18, 90, 75, 192, 160, 170, 200];
        let signature1 = bign_key.sign(&hash1, &mut rng).unwrap();
        let signature2 = bign_key.sign(&hash1, &mut rng).unwrap();
        // TODO: Check if it's fine that signature is the same despite function requiring `rng`.
        // assert_ne!(signature1, signature2);
        ...
        bign_key.verify(&bign_key.public_key, &hash1, &signature2).unwrap();
        // TODO: Check if that signature really is valid
        let valid_signature = [128, 169, 147, 27, 236, 72, 183, 38, 112, 93, 192, 204, 157, 177,
            127, 228, 2, 147, 162, 14, 71, 64, 66, 54, 29, 70, 226, 168, 97, 182, 39, 114, 139,
            13, 217, 194, 32, 71, 12, 182, 170, 21, 23, 111, 7, 100, 181, 155, 76, 129, 86, 244,
            74, 50, 115, 56, 109, 111, 80, 152, 219, 110, 211, 154, 31, 71, 113, 238, 99, 163,
            139, 243, 122, 121, 147, 217, 234, 207, 28, 55, 23, 74, 101, 215, 222, 228, 182, 114,
            137, 220, 117, 238, 79, 219, 98, 129];
        bign_key.verify(&bign_key.public_key, &hash1, &valid_signature).unwrap();
        let invalid_signature = [128, 169, 147, 27, 236, 72, 183, 38, 112, 93, 192, 204, 157, 177,
            127, 228, 2, 147, 162, 14, 71, 64, 66, 54, 29, 70, 226, 168, 97, 182, 39, 114, 139,
            13, 217, 194, 32, 71, 12, 182, 170, 21, 23, 111, 7, 100, 181, 155, 76, 129, 86, 244,
            74, 50, 115, 56, 109, 111, 80, 152, 219, 110, 211, 154, 31, 71, 113, 238, 99, 163,
            139, 243, 122, 121, 147, 217, 234, 207, 28, 55, 23, 74, 101, 215, 222, 228, 182, 114,
            137, 220, 117, 238, 79, 219, 98, 130];
        bign_key.verify(&bign_key.public_key, &hash1, &invalid_signature).unwrap_err();
        bign_params.validate().unwrap();
        bign_key.validate_keypair().unwrap();
        bign_key.validate_public_key().unwrap();
    ...
    fn test_diffie_hellman() {
        let bign_params = BignParameters::try_new(BignParametersConfiguration::B1).unwrap();
        bign_params.validate().unwrap();
        let mut rng = CtrRng::new([30, 138, 167, 215, 184, 0, 145, 201, 150, 217, 60, 134, 43,
            246, 123, 184, 131, 114, 192, 207, 197, 63, 222, 23, 132, 232, 188, 200, 109, 231, 16,
            47],
            None,
        );
        let mut bign_key = BignKey::try_new(
            bign_params.clone(),
            &mut rng,
        ).unwrap();
        bign_key.validate_keypair().unwrap();
        ...
        let mut rng2 = CtrRng::new([55, 205, 97, 114, 13, 64, 27, 233, 108, 221, 167, 134, 46,
            68, 196, 139, 94, 225, 130, 54, 137, 63, 246, 44, 116, 229, 10, 237, 102, 235, 142,
            112],
            None,
        );
        let mut bign_key2 = BignKey::try_new(
            bign_params.clone(),
            &mut rng2,
        ).unwrap();
