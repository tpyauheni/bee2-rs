...
mod tests {
    ...
    #[test]
    fn test_ctr() {
        let iv = [160, 78, 75, 213, 247, 122, 26, 130, 210, 165, 152, 151, 127, 68, 13, 191, 218,
            254, 187, 106, 158, 184, 100, 249, 28, 175, 182, 11, 49, 238, 79, 159];
        ...
        let correct_bytes = [147, 70, 47, 220, 55, 119, 251, 51, 229, 241, 36, 174, 253, 49,
            72, 21, 200, 101, 45, 138, 104, 221, 98, 163, 160, 193, 227, 145, 9, 125, 135, 40,
            194, 199, 58, 243, 133, 108, 177, 5, 195, 104, 34, 253, 70, 249, 135, 254, 229, 142];
        assert_eq!(bytes, correct_bytes);
        let mut bytes = [0; 10];
        let correct_bytes = [38, 63, 132, 211, 241, 76, 105, 55, 193, 59];
        let mut iv = [24, 86, 158, 178, 84, 209, 205, 75, 115, 252, 28, 234, 153, 108, 240, 32,
            213, 68, 123, 71, 73, 198, 180, 1, 91, 116, 158, 15, 46, 65, 143, 233];
        CtrRng::fill_buffer(
            &mut bytes,
            &[249, 185, 242, 52, 67, 251, 170, 132, 15, 99, 211, 198, 102, 161, 21, 158, 124, 185,
            156, 131, 232, 88, 234, 41, 1, 41, 218, 189, 147, 62, 245, 195],
            &mut iv,
        ).unwrap();
        assert_eq!(bytes, correct_bytes);
    }
    ...
    fn test_ctr_no_iv() {
        ...
        let mut bytes = [0; 50];
        ctr.next_buffer(&mut bytes);
        let correct_bytes = [211, 55, 107, 155, 251, 113, 255, 200, 127, 108, 230, 144, 223, 93,
            236, 116, 7, 149, 27, 218, 196, 123, 86, 177, 110, 104, 252, 167, 195, 185, 148, 127,
            12, 140, 102, 121, 241, 233, 203, 135, 22, 127, 59, 0, 251, 255, 143, 174, 95, 28];
        assert_eq!(bytes, correct_bytes);
    }
